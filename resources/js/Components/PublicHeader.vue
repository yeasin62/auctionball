...
<template>
    <header class="sticky top-0 z-30 bg-white/70 backdrop-blur-md border-b border-ink-200/40">
        <div class="mx-auto max-w-7xl px-3 sm:px-6 py-3 sm:py-4 flex items-center justify-between gap-2 sm:gap-3">
            <div class="flex min-w-0 items-center gap-2.5">
                ...
                <Link href="/" class="flex min-w-0 items-center gap-2.5">
                    ...
                    <span class="hidden min-[390px]:inline font-semibold text-[17px] tracking-tight">AuctionBall</span>
            ...
            <div class="flex shrink-0 items-center gap-1.5 sm:gap-2.5">
                ...
                <template v-if="authUser">
                    ...
                    <Link href="/dashboard-redirect" class="btn-primary whitespace-nowrap text-[12px] min-[390px]:text-[13px] sm:text-[14px] py-2 px-2.5 min-[390px]:px-3 sm:py-2.5 sm:px-4">
                        Dashboard
                    </Link>
                </template>
                <template v-else>
                    <Link href="/login" class="hidden sm:inline-flex btn-ghost text-[14px] py-2.5 px-4">{{ t('nav.log_in') }}</Link>
                    <Link href="/register" class="btn-primary whitespace-nowrap text-[12px] min-[390px]:text-[13px] sm:text-[14px] py-2 px-2.5 min-[390px]:px-3 sm:py-2.5 sm:px-4">{{ t('nav.start_free') }}</Link>
                </template>
            </div>
        </div>
    </header>
