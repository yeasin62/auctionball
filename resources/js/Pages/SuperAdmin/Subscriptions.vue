...
const forceRenew = async (s) => {
    if (! await confirm({
        title: `Force renewal for ${s.org}?`,
        description: `Subscription #${s.id} on the ${s.plan} plan via ${s.provider}. The renewal job runs immediately — billing will be attempted now.`,
        variant: 'warning',
...
const cancelSub = async (s) => {
    if (! await confirm({
        title: `Cancel subscription for ${s.org}?`,
        description: 'They keep access until the current period ends, then move to Free. They can re-subscribe any time.',
        variant: 'danger',
...
<template>
    <SuperAdminLayout title="Subscriptions">
        ...
        <div class="glass rounded-2xl p-4 mb-4 flex flex-wrap items-center gap-2">
            ...
            <span class="text-[12px] font-mono text-ink-500 ml-auto">{{ subs.total }} total</span>
        </div>
                ...
                <tbody class="divide-y divide-ink-100">
                    <tr v-for="s in subs.data" :key="s.id" class="hover:bg-white/40">
                        ...
                        <td class="px-4 py-2.5 font-mono text-[11.5px]">
                            <span :class="s.auto_renew ? 'text-emerald-700' : 'text-ink-400'">{{ s.auto_renew ? 'auto' : 'off' }}</span>
                        ...
                        <td class="px-4 py-2.5 text-right whitespace-nowrap">
                            <button @click="forceRenew(s)" class="text-[11.5px] text-brand-indigo hover:underline mr-3">Force renew</button>
                            <button v-if="s.status !== 'canceled' && s.status !== 'expired'"
                                    @click="cancelSub(s)" class="text-[11.5px] text-rose-500 hover:text-rose-700">Cancel</button>
                        </td>
                    </tr>
                    <tr v-if="subs.data.length === 0">
                        <td colspan="10" class="px-4 py-12 text-center text-[13px] text-ink-500">No subscriptions match the filters.</td>
                    </tr>
                </tbody>
            </table>
        </div>
