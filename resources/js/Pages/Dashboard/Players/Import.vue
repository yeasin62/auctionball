...
<template>
    <DashboardLayout title="Import players">
        <template #actions>
            <Link href="/dashboard/players" class="btn-ghost py-2 px-4 text-[13px]">Cancel</Link>
        </template>

        <div class="grid md:grid-cols-4 gap-4 mb-5">
            <div class="glass rounded-2xl p-4">
                <div class="font-mono text-[10.5px] tracking-widest text-ink-500">VALID ROWS</div>
                <div class="text-[28px] font-extrabold text-emerald-700 tracking-tight mt-1">{{ preview.valid_count }}</div>
            </div>
            <div class="glass rounded-2xl p-4">
                <div class="font-mono text-[10.5px] tracking-widest text-ink-500">INVALID</div>
            ...
            <div class="glass rounded-2xl p-4">
                <div class="font-mono text-[10.5px] tracking-widest text-ink-500">PLAN HEADROOM</div>
                <div class="text-[28px] font-extrabold tracking-tight mt-1">{{ preview.headroom }}</div>
                <div class="text-[11px] font-mono text-ink-500">used {{ used }} / {{ limits.players === 9223372036854775807 ? '∞' : limits.players }}</div>
            </div>
            <div class="glass rounded-2xl p-4 bg-gradient-to-br from-blue-50 to-violet-50 border border-violet-100">
                <div class="font-mono text-[10.5px] tracking-widest text-ink-500">WILL IMPORT</div>
                <div class="text-[28px] font-extrabold tracking-tight mt-1 text-grad">{{ preview.will_import }}</div>
                <div v-if="preview.skipped_for_limit > 0" class="text-[11px] font-mono text-amber-700">+{{ preview.skipped_for_limit }} skipped (plan limit)</div>
            </div>
        </div>

        <!-- Invalid rows -->
        <div v-if="preview.invalid_count > 0" class="glass rounded-2xl p-5 mb-5 border border-rose-200/60">
            <h3 class="text-[14px] font-bold text-rose-700 mb-3">{{ preview.invalid_count }} rows have errors and will be skipped</h3>
            <div class="overflow-x-auto">
                <table class="w-full text-[12px]">
                    <thead><tr class="text-left font-mono text-[10px] tracking-widest text-ink-500">
                        <th class="px-2 py-1.5">ROW</th><th class="px-2 py-1.5">NAME</th><th class="px-2 py-1.5">ERRORS</th>
                    </tr></thead>
                    <tbody>
                        <tr v-for="bad in preview.invalid" :key="bad.row" class="border-t border-ink-100">
                            ...
                            <td class="px-2 py-1.5">{{ bad.data.name || '<empty>' }}</td>
        ...
        <div class="glass rounded-2xl overflow-hidden mb-5">
            <div class="px-5 py-3 flex items-center justify-between">
                <h3 class="text-[14px] font-bold tracking-tight">Preview (first 50 rows)</h3>
                <span class="text-[11px] font-mono text-ink-500">{{ preview.valid.length }} of {{ preview.valid_count }} shown</span>
            </div>
            <table class="w-full text-[12.5px]">
                <thead class="bg-white/50">
                    <tr class="text-left font-mono text-[10px] tracking-widest text-ink-500">
                        ...
                        <th class="px-3 py-2">JERSEY</th>
                        <th class="px-3 py-2">BATTING</th>
                        <th class="px-3 py-2">BOWLING</th>
                    </tr>
                </thead>
        ...
        <div class="flex justify-end gap-2">
            <Link href="/dashboard/players" class="btn-ghost py-2.5 px-5 text-[13px]">Cancel</Link>
            <button @click="confirm" class="btn-primary py-2.5 px-5 text-[13px]"
                    :disabled="form.processing || preview.will_import === 0">
                {{ form.processing ? 'Importing…' : `Import ${preview.will_import} players` }}
            </button>
        </div>
    </DashboardLayout>
</template>
